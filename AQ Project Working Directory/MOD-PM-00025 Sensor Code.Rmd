...
timePlot(aq_df_filtered, pollutant = "pm10", scales= "free")
...
```{r}
mydata <- splitByDate(aq_df_met, dates= "1/8/2023", labels = c("With Construction", "Without Construction"))

myOutput <- timeVariation(mydata, pollutant = "pm10", group = "split.by")
...
aq_df_met$cooldate <- as.Date(aq_df_met$date, format = "%Y-%m-%d %H-%M-%S")
construction_df <-subset(aq_df_met, cooldate< '2023-8-1')
summary(construction_df)
```


```{r}
clean_df <-subset(aq_df_met, cooldate> '2023-8-1')
summary(clean_df)
```
