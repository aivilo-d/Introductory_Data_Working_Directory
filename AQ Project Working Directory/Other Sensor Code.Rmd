...
my_df <- mod_met_filtered %>% 
      filter(mod_met_filtered$sn == 'MOD-PM-00230')
...
my_df <-subset(my_df, cooldate> '2023-1-1')
```


```{r}
mydata <- splitByDate(my_df, dates= "1/6/2023", labels = c("With Construction", "Without Construction"))
...
```{r}
my_df$cooldate <- as.Date(my_df$date, format = "%Y-%m-%d %H-%M-%S")
construction_df <-subset(my_df, cooldate< '2023-6-1')
summary(construction_df)
```


```{r}
clean_df <-subset(my_df, cooldate> '2023-6-1')
summary(clean_df)
```
